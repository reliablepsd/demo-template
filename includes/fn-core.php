 ...
 * @return string
 */
function am_get_post_img( $size_in = array(), $post_id = null ) {
	...
	if ( has_post_thumbnail( $post_id ) ) :
		...
		$thumbnail_html = am_get_retina_img( $thumbnail[0], $class, $thumbnail[1], $thumbnail[2], $alt );
	endif;
 ...
 *
 */
function am_the_post_img( $size_in = array(), $post_id = null ) {

	echo am_get_post_img( $size_in, $post_id );
...
function am_get_retina_icon($image = array(), $args = array()){
	...
	if ($image && is_array($image)) :
		...
		$divider = $image['subtype'] == 'svg+xml' ? 1 : 2;

		$ready_img = '<img src="'.esc_url($url).'" width="'.esc_attr(min(round($image['width'] / $divider), $max_width)).'" '.($class? ' class="'.esc_attr($class).'" ' : '').' alt="'.($alt? esc_attr($alt) : '').'">';
